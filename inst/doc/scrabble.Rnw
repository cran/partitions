...
All software is available from CRAN, {\tt http://CRAN.r-project.org/}.
...
Figure~\ref{blocks} shows an example of this in action.  The top
...
\begin{figure}[htbp]
  \begin{center}
\includegraphics[width=5cm]{blocks}
\caption{A pictorial description of the algorithm used in \label{blocks}
  ...
  under consideration) in two consecutive configurations, the upper
  ...
  upper diagram completely fill the `N' column and this indicates that
  a maximum of two knights may be drawn.  The upper diagram thus
  ...
  above.  The lower diagram shows the next lexicographical
...
[the remainder of this paper uses only the first 10 letters of the
alphabet:

<<simple_problem>>=
  scrabble <- scrabble[1:10]
@ 

The user may delete the above line to determine the `real' scrabble
problem].
...
<<probOfRacks,print=FALSE,echo=TRUE,cache=TRUE>>=
...
<<maximal_prob,echo=TRUE,print=TRUE>>=
max(probs)
@ 

<<minimal_prob,echo=FALSE,print=FALSE>>=
mp <- min(probs)
a <- floor(log10(mp))
@ 

or just over once per~\Sexpr{ceiling(1/max(probs))} draws.  It is
interesting to note that the {\em least} probable rack is not unique:
there are exactly~\Sexpr{sum(probs==min(probs))} racks each with
